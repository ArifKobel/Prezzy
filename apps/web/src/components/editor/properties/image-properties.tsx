        ...
        {el.props?.src ? (
          <div className="space-y-2">
            ...
            <div className="flex gap-1.5">
              <button onClick={() => triggerImageUpload(el.id)} className="flex flex-1 items-center justify-center gap-1.5 rounded-md border border-border bg-surface px-2 py-1.5 font-sans text-[11px] font-medium text-foreground transition-all hover:bg-surface-container">
                <Upload className="size-3" /> Replace
              </button>
              <button onClick={() => showImageUrlDialog(el.id, el.props?.src ?? "")} className="flex flex-1 items-center justify-center gap-1.5 rounded-md border border-border bg-surface px-2 py-1.5 font-sans text-[11px] font-medium text-foreground transition-all hover:bg-surface-container">
                <Link className="size-3" /> URL
              </button>
              <button onClick={() => updateImageSrc({ id: el.id, src: "" })} className="flex items-center justify-center rounded-md border border-border bg-surface px-2 py-1.5 text-destructive transition-all hover:bg-destructive/10">
                <X className="size-3" />
              </button>
            </div>
          </div>
          ...
          <div
            ...
            onDrop={(e) => { e.preventDefault(); const file = e.dataTransfer.files?.[0]; if (file) handleImageUpload(file, el.id); }}
          >
            {uploadingImageId === el.id ? (
              ...
              <>
                ...
                <div className="flex flex-col items-center gap-1">
                  <button onClick={() => triggerImageUpload(el.id)} className="flex items-center gap-1.5 rounded-md bg-primary px-4 py-1.5 font-sans text-[11px] font-medium text-primary-foreground transition-all hover:bg-primary/90">
                    <Upload className="size-3" /> Upload Image
                  </button>
                  <button onClick={() => showImageUrlDialog(el.id)} className="font-sans text-[11px] text-muted-foreground transition-all hover:text-foreground">
                    or paste a URL
                  </button>
                </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-3 gap-1">
          {(["cover", "contain", "fill"] as const).map((fit) => (
            <button
              key={fit}
              onClick={() => updateProps({ id: el.id, props: { objectFit: fit } })}
              className={cn(
