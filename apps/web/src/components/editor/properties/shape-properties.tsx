...
export function ShapeProperties({ el }: { el: Doc<"slideElements"> }) {
    ...
    <>
      <section>
        ...
        <div className="grid grid-cols-4 gap-1">
          {SHAPE_TYPES.map((st) => {
            ...
            return (
              <button key={st.id} onClick={() => updateProps({ id: el._id, props: { shapeType: st.id } })} title={st.label}
                className={cn(
      ...
      <section>
        ...
        <div className="flex items-center gap-2">
          <input type="color" value={el.props?.color ?? "#d4c4b0"} onChange={(e) => updateProps({ id: el._id, props: { color: e.target.value } })} className="size-8 cursor-pointer rounded-md border border-border bg-transparent p-0.5" />
          <input type="text" value={el.props?.color ?? ""} placeholder="var(--color-secondary-container)" onChange={(e) => updateProps({ id: el._id, props: { color: e.target.value } })} className="flex-1 rounded-md border border-border bg-surface px-2 py-1.5 font-mono text-[11px] text-foreground outline-none placeholder:text-muted-foreground/40 focus:border-ring" />
        </div>
      </section>
