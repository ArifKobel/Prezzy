...
export function MirrorSection({ el }: { el: Doc<"slideElements"> }) {
  ...
  return (
    <section>
      ...
      <div className="flex gap-1.5">
        <button
          onClick={() => updateProps({ id: el._id, props: { flipX: !(el.props?.flipX ?? false) } })}
          className={cn(
        ...
        <button
          onClick={() => updateProps({ id: el._id, props: { flipY: !(el.props?.flipY ?? false) } })}
          className={cn(
            "flex flex-1 items-center justify-center gap-1.5 rounded-md px-2 py-2 font-sans text-[11px] font-medium transition-all",
