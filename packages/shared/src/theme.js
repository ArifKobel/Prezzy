...
exports.DEFAULT_SLIDE_THEME_ID = exports.SLIDE_THEME_PRESETS = void 0;
exports.mixHex = mixHex;
exports.resolveSlideTheme = resolveSlideTheme;
exports.SLIDE_THEME_PRESETS = [
    {
        id: "clean",
        label: "Clean",
        tokens: {
...
exports.DEFAULT_SLIDE_THEME_ID = "clean";
function presetById(id) {
    return exports.SLIDE_THEME_PRESETS.find((p) => p.id === id) ?? exports.SLIDE_THEME_PRESETS[0];
}
function clamp255(n) {
...
function legacyTokens(theme) {
    const t = {};
    if (theme.backgroundColor)
        t.bg = theme.backgroundColor;
    if (theme.surfaceColor)
        t.surface = theme.surfaceColor;
    else if (theme.secondaryColor)
        t.surface = theme.secondaryColor;
    if (theme.textColor)
        t.text = theme.textColor;
    if (theme.primaryColor)
        t.accent = theme.primaryColor;
    if (theme.headingFont)
        t.fontHeading = theme.headingFont;
    if (theme.bodyFont)
        t.fontBody = theme.bodyFont;
    return t;
}
function resolveSlideTheme(theme, slideBg) {
    const preset = presetById(theme?.base);
    const tokens = {
        ...preset.tokens,
        ...(theme ? legacyTokens(theme) : {}),
        ...theme?.overrides,
    };
    if (slideBg)
        tokens.bg = slideBg;
